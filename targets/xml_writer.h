...
namespace gr8 {
  ...
  protected:
    ...
    void do_access_specifier(gr8::access_specifier spec, int lvl);
    void do_basic_type(basic_type* type, int lvl, cdk::expression_node* value, int first);
    void do_null(int lvl);
    template<typename T>
    void process_literal(cdk::literal_node<T> * const node, int lvl) {
      os() << std::string(lvl, ' ') << "<" << node->label() << ">" << node->value() << "</" << node->label() << ">" << std::endl;
    }
    void do_function_helper(gr8::access_specifier accessSpec, basic_type* bType, std::string name, cdk::sequence_node* arguments, int lvl);
